...
# The selector string control
var selector_txt = null
...
#const sel_edit_rgx = "(?<=.(faces|edges|vertices)\\(\")(.*?)(?=\"\\))"
const sel_edit_rgx = "\\.(faces|edges|vertices).*"
...
func _ready():
	# Label for the selector string control
	var selector_lbl = Label.new()
	selector_lbl.set_text("Selector String")
	add_child(selector_lbl)

	# The control that holds the selector string
	selector_txt = LineEdit.new()
	selector_txt.size_flags_horizontal = selector_txt.SIZE_EXPAND_FILL
	add_child(selector_txt)
...
#	add_child(vertex_selector_txt)
	...
	# Workplane toggle
	wp_show_btn = CheckButton.new()
	wp_show_btn.set_text("Create Workplane: ")
	wp_show_btn.hint_tooltip = tr("SELECTOR_HIDE_SHOW_WP_BTN_HINT_TOOLTIP")
	wp_show_btn.connect("button_down", self, "_show_workplane")
	add_child(wp_show_btn)
...
func get_completed_template():
	...
	# Append the user approved/edited selector string
	completed += selector_txt.get_text()
...
func set_values_from_string(text_line):
	...
	# Selector string
	rgx.compile(sel_edit_rgx)
	var res = rgx.search(text_line)
	if res:
		# Set the face selector controls
		var sel = res.get_string()
		selector_txt.set_text(sel)

	# Workplane edit selector
	rgx.compile(wp_edit_rgx)
	res = rgx.search(text_line)
	if res:
		wp_show_btn.pressed = true
		wp_ctrl.show()

		# Remove the workplane text from the selector text line
		selector_txt.set_text(selector_txt.get_text().replace(res.get_string(), "").replace(".workplane()", ""))
